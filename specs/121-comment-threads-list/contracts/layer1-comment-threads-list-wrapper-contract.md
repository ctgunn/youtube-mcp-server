# Contract: YT-121 Layer 1 `commentThreads.list` Wrapper Contract

## Purpose

Define the internal wrapper contract for YouTube Data API `commentThreads.list` so maintainers can review selector behavior, quota visibility, optional modifier boundaries, and normalized result handling before implementation details are inspected.
...
- Internal-only Layer 1 wrapper behavior for `commentThreads.list`
...
The `commentThreads.list` wrapper must expose:
...
- `quota_cost` as `1`
- maintainer-facing notes describing selector rules, optional modifiers, access implications, and empty-result guidance relevant to comment-thread retrieval
...
- which selectors are supported for this slice (`videoId`, `allThreadsRelatedToChannelId`, and `id`)
- that exactly one selector may be active in a supported request
- which optional near-raw retrieval fields are accepted (`pageToken`, `maxResults`, `order`, `searchTerms`, `textFormat`)
...
- valid requests return normalized success, including empty item lists when no comment threads match
...
- identify `commentThreads.list` identity, quota cost, and supported selector set in one review pass
- determine supported selectors and selector exclusivity rules without reading implementation code
- understand that video-based, channel-related, and ID-based lookup are separate supported paths
...
- YT-121 extends YT-101 and YT-102 foundations instead of replacing them

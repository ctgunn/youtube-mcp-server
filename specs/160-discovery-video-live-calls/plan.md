...
**Performance Goals**: Preserve the configured per-attempt timeout; use bounded full-jitter exponential backoff only for idempotent retries; transfer resumable video uploads in configurable 256 KiB-aligned chunks
...
- The media operations are `thumbnails.set`, `videos.insert`, and `watermarks.set`. They use the shared transport only. Existing `videos.insert` validation exposes `multipart` and `resumable` upload-mode values; the shared transport implements the resumable-session protocol and Google upload endpoints without adding resource-specific upload code.

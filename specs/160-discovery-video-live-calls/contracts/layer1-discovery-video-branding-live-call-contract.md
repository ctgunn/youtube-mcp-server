...
5. The concrete transport preserves metadata-defined target, method, parameters, body, and media form; attaches the selected credential; and returns the existing normalized success or normalized failure.
...
| Media operation | Preserve existing media and metadata validation and submit raw-media or multipart content through the shared transport. No resource-specific media client or resumable-session protocol is added by this retrofit. |

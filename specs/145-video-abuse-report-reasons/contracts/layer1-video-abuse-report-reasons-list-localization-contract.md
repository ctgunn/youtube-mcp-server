# Contract: YT-145 Layer 1 `videoAbuseReportReasons.list` Localization Contract

## Purpose

Define how maintainers and future higher-layer authors should interpret `videoAbuseReportReasons.list` localization lookup rules, empty-result handling, and invalid-request boundaries when reusing the internal Layer 1 wrapper.
...
- hidden implementation details to infer whether a localized abuse-reason view is supported
...
- valid `videoAbuseReportReasons.list` requests may return zero items without becoming failures
...
Representative proof for YT-145 must show:

- maintainers can identify localized abuse-reason lookup behavior in one review pass
- supported display-language lookup rules are documented clearly enough for later video-reporting features
...
- abuse-report-reason retrieval handling remains on the success path for valid requests that return zero or more items

Required coverage:

- contract checks for feature artifacts under `/Users/ctgunn/Projects/youtube-mcp-server/specs/145-video-abuse-report-reasons/contracts/`
- unit coverage for request validation and metadata exposure
- transport coverage showing `GET` request construction for `videoAbuseReportReasons.list`

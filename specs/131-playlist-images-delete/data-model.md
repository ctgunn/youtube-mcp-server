# Data Model: YT-131 Layer 1 Endpoint `playlistImages.delete`

## Entity: Playlist Images Delete Wrapper Contract

**Purpose**: Describes the internal Layer 1 wrapper contract for `playlistImages.delete` so maintainers can review endpoint identity, quota cost, delete-input rules, OAuth behavior, and normalized delete boundaries before implementation details are inspected.
...
- `request_shape`: Supported request fields and endpoint-specific validators for playlist-image removal
...
- Uses one `Playlist Image Delete Request`
- Applies one `Playlist Image Delete Access Profile`
- Produces one `Playlist Image Delete Result`

## Entity: Playlist Image Delete Request

**Purpose**: Represents one supported `playlistImages.delete` request.

**Fields**:

- `id`: Required target playlist-image identifier for one delete attempt
- `delete_scope`: Maintainer-facing label describing that the supported boundary is one playlist-image deletion rather than bulk removal
...
- Every supported request must include one non-empty target playlist-image identifier
- The request must remain deterministic and must not require `part`, `body`, or `media` for this slice
...
- Requires one `Playlist Image Delete Access Profile`
- Produces one `Playlist Image Delete Result`

## Entity: Playlist Image Delete Access Profile

**Purpose**: Captures the authorization requirement and failure-categorization expectations for one `playlistImages.delete` request path.

**Fields**:

- `auth_mode`: OAuth-required label for the wrapper
- `requires_authorized_access`: Whether the request needs authorized playlist-image delete access
...
- Supported `playlistImages.delete` paths must require authorized access
...
- Applied by `Playlist Images Delete Wrapper Contract`
- Used to interpret `Playlist Image Delete Result`

## Entity: Playlist Image Delete Target State

**Purpose**: Defines the reviewable status of the targeted playlist image at delete time so downstream layers can interpret unavailable-resource outcomes clearly.

**Fields**:

- `target_id`: The playlist-image identifier supplied for deletion
...
- The target state must remain reviewable enough that maintainers can distinguish missing or malformed identifiers from unavailable playlist images
...
- Interpreted alongside one `Playlist Image Delete Request`
- Influences one `Playlist Image Delete Result`

## Entity: Playlist Image Delete Result

**Purpose**: Represents normalized acknowledgment output for one valid `playlistImages.delete` request.

**Fields**:

- `playlist_image_id`: Stable target playlist-image identifier preserved for downstream review or mapping
...
- Successful results must preserve enough delete context for downstream callers to confirm which playlist image was removed
- Failure outcomes must preserve distinctions required by downstream callers

**Relationships**:

- Produced by one `Playlist Images Delete Wrapper Contract`
- Interpreted alongside one `Playlist Image Delete Access Profile`
...
1. Caller submits one delete request with a playlist-image identifier

# Data Model: YT-230 Layer 2 Tool `playlistImages_update`

## Playlist Images Update Tool

**Purpose**: Public Layer 2 MCP tool for OAuth-backed playlist-image update through `playlistImages.update`.

**Fields**:
- `toolName`: Must be `playlistImages_update`.
- `upstreamResource`: Must be `playlistImages`.
- `upstreamMethod`: Must be `update`.
- `operationKey`: Must be `playlistImages.update`.
- `quotaCost`: Must be `50`.
- `authMode`: Must be `oauth_required`.
- `availabilityState`: Active with OAuth-required playlist-image update and media-upload caveats.

**Relationships**:
- Uses one Playlist Images Update Request.
- Produces one Playlist Images Update Result or one safe Playlist Images Update Error.
- Depends on the YT-130 Layer 1 `playlistImages.update` wrapper.
...
## Playlist Images Update Request

**Purpose**: Caller-supplied input for one playlist-image update operation.

**Fields**:
- `part`: Required string. Determines which playlist image resource sections are returned after update.
- `body`: Required object. Carries playlist-image update metadata, including the target playlist image identity and supported owning playlist context.
- `media`: Required object. Carries supported replacement media-upload content or descriptors for the playlist image.

**Relationships**:
- Part Selection determines returned playlist-image fields.
- Playlist Image Update Metadata defines the target and update context.
- Playlist Image Upload Content supplies the replacement image content required for the mutation.
...
- Reject `body` metadata that lacks the target playlist image identifier.
- Reject `body` metadata that lacks required playlist-image update fields such as owning playlist context.
...
- Reject API-key-only or missing OAuth access before treating the request as a successful playlist-image update.

## Part Selection

**Purpose**: Defines which playlist-image resource sections are requested in the update result.
...
## Playlist Image Update Metadata

**Purpose**: Represents the playlist-image metadata supplied in `body`.

**Fields**:
- `id`: Required playlist image identifier for the existing image being updated.
- `snippet`: Required supported metadata section for playlist-image update.
- `playlistId`: Owning playlist context inside `snippet` when required by the supported metadata contract.
- Other supported playlist-image update fields only when explicitly allowed by the endpoint contract.

**Validation Rules**:
- Must be object-shaped and non-empty.
- Must include required target identity and update metadata.
...
**Fields**:
- `mimeType`: Required media type descriptor for the replacement image.
...
**Purpose**: Represents the OAuth-backed access needed to update playlist image data.
...
- Missing OAuth and insufficient OAuth access must be distinguishable from malformed requests and upstream update failures.
- Public results and errors must not expose secrets or sensitive authorization details.

## Updated Playlist Image Resource

**Purpose**: Represents the playlist image record returned after successful update.

**Fields**:
- `kind`: Returned resource kind, when present.
- `etag`: Returned resource etag, when present.
- `id`: Updated playlist-image identifier, when present.
...
## Playlist Images Update Result

**Purpose**: Successful near-raw response wrapper for playlist-image update.

**Fields**:
- `endpoint`: `playlistImages.update`.
- `quotaCost`: `50`.
- `requestedParts`: Normalized part selection.
- `bodyContext`: Safe metadata summary for the update request.
- `mediaContext`: Safe upload summary for the update request.
- `auth`: Safe auth-mode summary.
- `item`: Updated playlist image resource, when returned as a resource object.
- `kind`: Upstream response kind, when returned.
- `etag`: Upstream etag, when returned.
- `id`: Updated playlist-image identifier, when returned.
- `snippet`: Upstream snippet, when returned.

**Validation Rules**:
- Returned fields must be preserved without adding playlist image listing, insertion, deletion, thumbnail activation, playlist management, analytics, ranking, summarization, enrichment, recommendation, or heuristic classification.
- Missing optional upstream fields must not be fabricated.
- Raw media content must not be echoed in result context.

## Playlist Images Update Error

**Purpose**: Safe caller-facing failure for invalid, inaccessible, quota, missing-resource, media eligibility, unavailable, or unexpected playlist-image update outcomes.
...
- Must distinguish malformed input, auth failure, quota failure, media eligibility failure, missing or inaccessible resources, upstream update failure, and successful update.
...
**Purpose**: Caller-facing explanation of what the low-level playlist-image update tool does not do.

**Fields**:
- `unsupportedBehaviors`: Playlist image listing, insertion, deletion, thumbnail management, playlist management, playlist-item expansion, analytics, ranking, summarization, enrichment, recommendation, and cross-endpoint aggregation.

# Contract: YT-125 Layer 1 `i18nRegions.list` Region Contract

## Purpose

Define how maintainers and future higher-layer authors should interpret `i18nRegions.list` region lookup rules, empty-result handling, and invalid-request boundaries when reusing the internal Layer 1 wrapper.
...
- hidden implementation details to infer whether a localization-region view is supported
...
- the supported lookup path for this slice uses one optional `hl` value per request
- every supported request must include non-empty `part`
- omitting `hl` remains valid and preserves upstream default display-language behavior
...
- valid `i18nRegions.list` requests may return zero items without becoming failures
...
- callers may omit `hl`; missing `hl` must not become an invalid-request condition
...
Representative proof for YT-125 must show:

- maintainers can identify region-lookup behavior in one review pass
...
- localization-region retrieval handling remains on the success path for valid requests that return zero or more items

Required coverage:

- contract checks for feature artifacts under `/Users/ctgunn/Projects/youtube-mcp-server/specs/125-i18n-regions-list/contracts/`
- unit coverage for request validation and metadata exposure
- transport coverage showing `GET` request construction for `i18nRegions.list`

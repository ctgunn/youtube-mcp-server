# Contract: YT-137 Layer 1 `playlists.insert` Wrapper Contract

## Purpose

Define the internal wrapper contract that the repository will use for the YouTube Data API `playlists.insert` endpoint so maintainers can review endpoint identity, quota behavior, writable request scope, and downstream reuse expectations before implementation details are inspected.
...
- Internal-only Layer 1 wrapper behavior for `playlists.insert`
- Maintainer-visible endpoint identity and quota cost
- Supported request-shape and playlist-creation boundary rules
- Review surfaces used by later playlist planning

This contract does not define a public MCP tool, hosted-route behavior, or MCP transport changes.

## Required Wrapper Metadata

The `playlists.insert` wrapper must expose:
...
Maintainer-facing review surfaces should also keep the minimum creation boundary visible, including the requirement that valid requests include a writable `snippet` carrying the playlist title used to create the playlist.
...
- which request field carries the minimum `body` metadata payload for playlist creation
- which nested writable field identifies the minimum required playlist title
- that `part=snippet` and the supported writable `snippet` form the minimum supported create boundary for this slice
- which optional writable fields are supported and which are outside the promised contract, including unsupported `body.snippet.description`, `body.status`, and `body.localization` inputs in this slice
- that unsupported field combinations are not part of the promised Layer 1 contract

The request contract must remain deterministic. A valid request must include the required playlist-title input, and unsupported combinations may not be silently rewritten into another request mode.

## Response Contract Expectations

The wrapper must preserve the current shared executor success and failure split:

- valid requests that create a playlist resource are successful
...
- identify the wrapper as `playlists.insert` in one review pass
...
- understand the supported boundary for one writable playlist creation request
...
- consumer-facing checks showing higher layers can summarize `playlists.insert` contract details without losing quota, auth, or writable-boundary visibility

## Invariants

- YT-137 extends the existing YT-101 and YT-102 Layer 1 foundation rather than replacing it

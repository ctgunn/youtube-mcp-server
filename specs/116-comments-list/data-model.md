# Data Model: YT-116 Layer 1 Endpoint `comments.list`

## Entity: Comments List Wrapper Contract

**Purpose**: Describes the internal Layer 1 wrapper contract for `comments.list` so maintainers can review endpoint identity, quota cost, selector rules, access behavior, optional modifiers, and result boundaries before implementation details are inspected.
...
- `request_shape`: Supported request fields and endpoint-specific validators for comment retrieval
...
- Contract notes must include direct-comment lookup, parent-comment reply lookup, and empty-result interpretation guidance
- Selector guidance must keep unsupported combinations and undocumented fields explicit

**Relationships**:

- Uses one `Comments List Request`
- Applies one `Comment Lookup Access Profile`
- Produces one `Comments List Result`

## Entity: Comments List Request

**Purpose**: Represents one supported `comments.list` request.

**Fields**:

- `part`: Requested response sections for one retrieval attempt
- `id`: Optional direct-comment selector for one or more comment identifiers
- `parentId`: Optional parent-comment selector for reply retrieval
...
- Every supported request must include exactly one active selector from `id` or `parentId`
...
- Requires one `Comment Lookup Access Profile`
- Produces one `Comments List Result`

## Entity: Comment Lookup Mode

**Purpose**: Captures the supported retrieval path selected for one `comments.list` request.

**Fields**:

- `selector_name`: The selector field that activates the path, expected to be `id` or `parentId`
- `lookup_goal`: Whether the request fetches direct comment resources or reply comments
- `selector_scope`: Whether the request targets one or more comment identifiers or replies under one parent comment
...
- Activated by one `Comments List Request`
- Informs one `Comment Lookup Access Profile`

## Entity: Comment Lookup Access Profile

**Purpose**: Captures the auth expectation and failure categorization for one `comments.list` request path.
...
- Applied by `Comments List Wrapper Contract`
- Used to interpret `Comments List Result`

## Entity: Comments List Result

**Purpose**: Represents normalized output for one valid `comments.list` request.

**Fields**:

- `items`: Returned comment resources or reply resources for the request
...
- Produced by one `Comments List Wrapper Contract`
- Interpreted alongside one `Comment Lookup Mode`
...
1. Caller submits one `comments.list` request with `part` and one selector

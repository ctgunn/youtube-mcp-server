# Contract: YT-138 Layer 1 `playlists.update` Wrapper Contract

## Purpose

Define the internal wrapper contract that the repository will use for the YouTube Data API `playlists.update` endpoint so maintainers can review endpoint identity, quota behavior, writable request scope, and downstream reuse expectations before implementation details are inspected.
...
- Internal-only Layer 1 wrapper behavior for `playlists.update`
- Maintainer-visible endpoint identity and quota cost
- Supported request-shape and playlist update boundary rules
- Review surfaces used by later playlist update planning

This contract does not define a public MCP tool, hosted-route behavior, or MCP transport changes.

## Required Wrapper Metadata

The `playlists.update` wrapper must expose:
...
Maintainer-facing review surfaces should also keep the minimum update boundary visible, including the requirement that valid requests include the existing playlist identifier plus a writable `snippet` carrying the playlist title for the update.
...
- which request field carries the minimum `body` metadata payload for playlist updates
- which nested writable fields identify the existing playlist and the guaranteed writable title input
- that `part=snippet` and the supported writable `snippet` form the minimum supported update boundary for this slice
- which optional writable fields are supported and which are outside the promised contract, including unsupported `body.snippet.description`, `body.status`, and `body.localizations` inputs in this slice
...
- valid requests that update a playlist resource are successful
...
- identify the wrapper as `playlists.update` in one review pass
...
- understand the supported boundary for one writable playlist update request
...
- consumer-facing checks showing higher layers can summarize `playlists.update` contract details without losing quota, auth, or writable-boundary visibility

## Invariants

- YT-138 extends the existing YT-101 and YT-102 Layer 1 foundation rather than replacing it

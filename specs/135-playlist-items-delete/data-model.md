# Data Model: YT-135 Layer 1 Endpoint `playlistItems.delete`

## Entity: Playlist Items Delete Wrapper Contract

**Purpose**: Describes the internal Layer 1 wrapper contract for `playlistItems.delete` so maintainers can review endpoint identity, quota cost, delete-input rules, OAuth behavior, and normalized delete boundaries before implementation details are inspected.
...
- `request_shape`: Supported request fields and endpoint-specific validators for playlist-item removal
...
- Uses one `Playlist Item Delete Request`
- Applies one `Playlist Item Delete Access Profile`
- Produces one `Playlist Item Delete Result`

## Entity: Playlist Item Delete Request

**Purpose**: Represents one supported `playlistItems.delete` request.

**Fields**:

- `id`: Required target playlist-item identifier for one delete attempt
- `delete_scope`: Maintainer-facing label describing that the supported boundary is one playlist-item deletion rather than bulk removal
...
- Every supported request must include one non-empty target playlist-item identifier
...
- Requires one `Playlist Item Delete Access Profile`
- Produces one `Playlist Item Delete Result`

## Entity: Playlist Item Delete Access Profile

**Purpose**: Captures the authorization requirement and failure-categorization expectations for one `playlistItems.delete` request path.

**Fields**:

- `auth_mode`: OAuth-required label for the wrapper
- `requires_authorized_access`: Whether the request needs authorized playlist-item delete access
...
- Supported `playlistItems.delete` paths must require authorized access
...
- Applied by `Playlist Items Delete Wrapper Contract`
- Used to interpret `Playlist Item Delete Result`

## Entity: Playlist Item Delete Target State

**Purpose**: Defines the reviewable status of the targeted playlist item at delete time so downstream layers can interpret unavailable-resource outcomes clearly.

**Fields**:

- `target_id`: The playlist-item identifier supplied for deletion
...
- The target state must remain reviewable enough that maintainers can distinguish missing or malformed identifiers from unavailable playlist items
...
- Interpreted alongside one `Playlist Item Delete Request`
- Influences one `Playlist Item Delete Result`

## Entity: Playlist Item Delete Result

**Purpose**: Represents normalized acknowledgment output for one valid `playlistItems.delete` request.

**Fields**:

- `playlist_item_id`: Stable target playlist-item identifier preserved for downstream review or mapping
...
- Successful results must preserve enough delete context for downstream callers to confirm which playlist item was removed
- Failure outcomes must preserve distinctions required by downstream callers

**Relationships**:

- Produced by one `Playlist Items Delete Wrapper Contract`
- Interpreted alongside one `Playlist Item Delete Access Profile`
...
1. Caller submits one delete request with a playlist-item identifier

# Feature Specification: Layer 1 Live Calls for Channel and Community Resources

**Feature Branch**: `158-channel-community-live-calls`  
**Created**: 2026-08-02  
**Status**: Draft  
**Input**: User description: "Read the PRD.md to get an overview of the project and its goals for context. Then, work on the requirements for YT-158, as outlined in spec-kit-seed.md."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Receive Live Channel and Community Results (Priority: P1)

As an agent developer, I can invoke any supported channel or community operation through its configured default path and receive a result from the live YouTube service instead of a representative response.

**Why this priority**: Trustworthy live data is required before public tools built on these operations can support channel and community workflows.
...
3. **Given** an unsupported selector or invalid request input, **When** the operation is invoked, **Then** its existing validation outcome is returned before a live request is made.

---

### User Story 2 - Apply the Correct Authorization and Request Form (Priority: P2)

As an operator, I can rely on each channel and community operation to use the authorization rule and request details already declared for that operation when it accesses live data.

**Why this priority**: The operations span read, create, update, moderation, deletion, and media-bearing work, so an incorrect authorization choice or request form can block a workflow or change the wrong resource.

**Independent Test**: Exercise a read operation, a mutation, and a media-bearing operation with controlled live-runtime requests, and verify the declared authorization mode, request method, target, parameters, body, or upload form for each.
...
2. **Given** an operation that creates, updates, moderates, deletes, or uploads media, **When** it is invoked, **Then** its established request details are passed to the live execution path without changing the public wrapper contract.
...
1. **Given** a configured public-tool flow for activities, captions, channel banners, channels, channel sections, comments, or comment threads, **When** it is invoked, **Then** it reaches the corresponding live wrapper path.
...
- An operation includes selectors, query parameters, a structured body, or a media upload; the existing request validation and shaping rules remain in force for that operation.
...
- **Required test levels**: Unit tests for wrapper request construction and authorization selection; integration tests with controlled live-runtime responses; contract tests for preserved wrapper results and normalized failures; and configured public-tool flow tests for activities, captions, channel banners, channels, channel sections, comments, and comment threads.
...
- **FR-001**: The system MUST make the configured default invocation of each listed channel and community wrapper use the shared live execution path and MUST NOT return a representative default executor result or static successful result.
- **FR-002**: The scope of this feature MUST include exactly these existing operations: `activities.list`; `captions.list`, `captions.insert`, `captions.update`, `captions.download`, and `captions.delete`; `channelBanners.insert`; `channels.list` and `channels.update`; `channelSections.list`, `channelSections.insert`, `channelSections.update`, and `channelSections.delete`; `comments.list`, `comments.insert`, `comments.update`, `comments.setModerationStatus`, and `comments.delete`; and `commentThreads.list` and `commentThreads.insert`.
- **FR-003**: Each in-scope wrapper MUST preserve its existing public contract, endpoint metadata, selector validation, quota documentation, authorization requirement, and response normalization when it uses live execution.
...
- **FR-006**: When live configuration or the required authorization is unavailable or unusable, each in-scope wrapper MUST return the established normalized configuration or authorization failure and MUST NOT return representative data.
- **FR-007**: Each in-scope wrapper MUST map successful live responses and upstream failures through its existing response-normalization and error-normalization behavior.
- **FR-008**: The system MUST provide request-level automated coverage for every in-scope operation's request target, method, parameters, authorization mode, body or upload form where applicable, successful-result mapping, and normalized upstream failure behavior.
- **FR-009**: The system MUST demonstrate at least one configured public-tool flow reaches the live wrapper path for each affected resource family: activities, captions, channel banners, channels, channel sections, comments, and comment threads.
- **FR-010**: The feature MUST retrofit existing wrappers only and MUST NOT add new endpoint inventory, change existing wrapper contracts, or create a separate direct upstream path for affected public tools.
...
- **Channel and Community Wrapper**: An existing operation that retrieves or changes activities, captions, channel banners, channels, channel sections, comments, or comment threads.
...
- The existing wrapper contracts, metadata, selectors, quota documentation, authorization rules, and normalizers delivered by YT-103 through YT-122 are the compatibility baseline for this feature.
...
- **SC-001**: 20 of 20 in-scope wrapper operations demonstrate in automated configured-default tests that they select the live execution path and do not return a representative or static successful result.
- **SC-002**: 20 of 20 in-scope operations have request-level automated coverage for the applicable request target, method, parameters, authorization mode, body or upload form, successful-result mapping, and normalized upstream failure.
- **SC-003**: 100% of affected wrapper compatibility tests continue to pass for public contracts, metadata, selector validation, quota documentation, authorization rules, and normalized outcomes.
...
- **SC-006**: A review of automated test evidence finds zero configured-default paths among the 20 in-scope operations that use a resource-specific direct execution path or a representative default result.

...
def _valid_videos_get_rating_arguments(**overrides):
    """Build a valid ``videos_getRating`` request for unit tests.

    :param overrides: Field overrides for the default request.
    :return: A valid rating lookup request with any overrides applied.
    """
...
def _valid_videos_delete_arguments(**overrides):
    """Build a valid ``videos_delete`` request for unit tests.

    :param overrides: Field overrides for the default request.
    :return: A valid delete request with any overrides applied.
    """
    arguments = {"id": " abc123 "}
    arguments.update(overrides)
    return arguments
...
def test_validate_videos_get_rating_accepts_single_multi_and_delegated_lookups():
    ...
    assert validate_videos_get_rating_arguments(
        _valid_videos_get_rating_arguments(onBehalfOfContentOwner=" owner-123 ")
    ) == {"id": "abc123,def456", "onBehalfOfContentOwner": "owner-123"}


def test_map_videos_get_rating_result_returns_per_video_lookup_outcomes():
    """Map successful rating lookup responses without fabricating video details."""
    from mcp_server.tools.youtube_common.videos import map_videos_get_rating_result

    result = map_videos_get_rating_result(
        {
            "kind": "youtube#videoGetRatingResponse",
            "etag": "etag-rating",
            "items": [
                {"videoId": "abc123", "rating": "like"},
                {"videoId": "def456", "rating": "none"},
                {"videoId": "ghi789", "rating": "unspecified"},
            ],
        },
    ...
    assert result["quotaCost"] == 1
    assert result["lookup"] == {"requestedIds": ["abc123", "def456", "ghi789"], "resultCount": 3}
    assert result["auth"] == {"mode": "oauth_required", "path": "restricted"}
    assert result["availability"] == {"state": "active"}
    assert result["kind"] == "youtube#videoGetRatingResponse"
    assert result["etag"] == "etag-rating"
    assert result["items"] == [
        {"videoId": "abc123", "rating": "like", "isRated": True, "isUnrated": False},
        {"videoId": "def456", "rating": "none", "isRated": False, "isUnrated": True},
        {"videoId": "ghi789", "rating": "unspecified", "isRated": False, "isUnrated": True},
    ]
    assert "mutation" not in result
    assert "statistics" not in result


def test_videos_get_rating_handler_calls_layer1_once_with_oauth():
    """Execute the Layer 1 get-rating wrapper once with OAuth-required credentials."""
    from mcp_server.tools.youtube_common.videos import build_videos_get_rating_handler

    payload = {"items": [{"videoId": "abc123", "rating": "dislike"}, {"videoId": "def456", "rating": "none"}]}
    wrapper = RecordingWrapper(payload=payload)
    handler = build_videos_get_rating_handler(wrapper=wrapper, oauth_token="local-oauth")

    result = handler(_valid_videos_get_rating_arguments())

    assert result["endpoint"] == "videos.getRating"
    assert result["lookup"]["requestedIds"] == ["abc123", "def456"]
    assert result["items"][0]["rating"] == "dislike"
...
def test_videos_get_rating_handler_rejects_missing_oauth_before_layer1_execution():
    ...
    with pytest.raises(VideosGetRatingToolError) as exc_info:
        handler(_valid_videos_get_rating_arguments())
...
def test_validate_videos_delete_accepts_one_normalized_target_id():
    """Normalize a valid video delete target without changing identity semantics."""
    from mcp_server.tools.youtube_common.videos import validate_videos_delete_arguments

    assert validate_videos_delete_arguments(_valid_videos_delete_arguments()) == {"id": "abc123"}


def test_map_videos_delete_result_returns_no_content_acknowledgment():
    """Map successful no-content delete responses without fabricating video details."""
    from mcp_server.tools.youtube_common.videos import map_videos_delete_result

    result = map_videos_delete_result(
        {"sourceOperation": "videos.delete", "isDeleted": True},
        _valid_videos_delete_arguments(),
    )
    ...
    assert result == {
        ...
        "status": {"code": 204, "body": "none"},
        "sourceOperation": "videos.delete",
    }
    assert "item" not in result
    assert "metadata" not in result


def test_videos_delete_handler_calls_layer1_once_with_oauth():
    """Execute the Layer 1 delete wrapper once with OAuth-required credentials."""
    from mcp_server.tools.youtube_common.videos import build_videos_delete_handler

    wrapper = RecordingWrapper(payload={"sourceOperation": "videos.delete", "isDeleted": True})
    handler = build_videos_delete_handler(wrapper=wrapper, oauth_token="local-oauth")

    result = handler(_valid_videos_delete_arguments())
...
def test_videos_delete_handler_rejects_missing_oauth_before_layer1_execution():
    ...
    with pytest.raises(VideosDeleteToolError) as exc_info:
        handler(_valid_videos_delete_arguments())

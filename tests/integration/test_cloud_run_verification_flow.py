...
class CloudRunVerificationFlowIntegrationTests(unittest.TestCase):
    ...
    def test_ready_app_passes_full_hosted_verification(self):
        ...
        self.assertEqual(by_name["fetch-tool-call-legacy-shape"].result, "pass")
    ...
    def test_verification_evidence_is_written(self):
        ...
        self.assertIn("checkName: search-tool-call-empty", content)
        self.assertIn("checkName: fetch-tool-call-legacy-shape", content)
    ...
    def test_hosted_verification_uses_session_aware_transport(self):
        ...
        invalid = next(check for check in run.checks if check.check_name == "initialize-invalid-no-session")
        post = next(check for check in run.checks if check.check_name == "session-post-continuation")
        replay = next(check for check in run.checks if check.check_name == "session-reconnect")
    ...
    def test_verification_distinguishes_cloud_reachability_denial_from_mcp_denial(self):
        ...
        failing = next(check for check in mcp_run.checks if check.result == "fail")
    ...
    def test_verification_reports_secret_access_failure_layer(self):
        ...
        run = run_hosted_verification(self._revision(), requester=self._hosted_requester(app))
        failing = next(check for check in run.checks if check.result == "fail")

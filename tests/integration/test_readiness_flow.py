...
class ReadinessFlowTests(unittest.TestCase):
    ...
    def test_hosted_readyz_status_matches_ready_and_not_ready_states(self):
        ready_app = create_app(env={"MCP_ENVIRONMENT": "dev"})
        ready_response = execute_hosted_request(ready_app, method="GET", path="/readyz")
        ...
        not_ready_response = execute_hosted_request(not_ready_app, method="GET", path="/readyz")

...
def test_default_registry_includes_concrete_timestamped_caption_tool():
    """Discover the default concrete timestamped-caption descriptor."""
    dispatcher = InMemoryToolDispatcher()
    listed = {tool["name"]: tool for tool in dispatcher.list_tools()}

    assert "transcripts_getTimestampedCaptions" in listed
    metadata = listed["transcripts_getTimestampedCaptions"]["metadata"]
    assert metadata["compositionBoundary"]["kind"] == "timestamped_caption_retrieval"
...
def test_default_registry_includes_channel_details_tool():
    ...
    assert "channels_getChannel" in listed
    assert listed["channels_getChannel"]["metadata"]["compositionBoundary"]["kind"] == "normalized_enrichment"
...
def test_default_registry_includes_batch_channel_details_tool():
    ...
    assert "channels_getChannels" in listed
    assert listed["channels_getChannels"]["metadata"]["compositionBoundary"]["kind"] == "normalized_batch_enrichment"
    assert "representativeOnly" not in listed["channels_getChannels"]["metadata"]
...
def test_default_registry_includes_and_executes_channel_search_tool():
    """Discover and invoke the default concrete channel-search tool."""
    dispatcher = InMemoryToolDispatcher()
    listed = {tool["name"]: tool for tool in dispatcher.list_tools()}

    result = dispatcher.call_tool("channels_searchChannels", {"query": "mcp server"})

    assert "channels_searchChannels" in listed
    assert listed["channels_searchChannels"]["metadata"]["compositionBoundary"]["kind"] == "ranked_enrichment"
...
def test_default_registry_includes_playlist_item_retrieval_tool():
    ...
    assert "playlists_getPlaylistItems" in listed
    assert listed["playlists_getPlaylistItems"]["metadata"]["compositionBoundary"]["kind"] == "source_ordered_collection"
    assert "representativeOnly" not in listed["playlists_getPlaylistItems"]["metadata"]
...
def test_default_registry_includes_executable_videos_rate_tool():
    """Register ``videos_rate`` by default with safe metadata and handler."""
    dispatcher = InMemoryToolDispatcher()
    listed = {tool["name"]: tool for tool in dispatcher.list_tools()}

    assert "videos_rate" in listed
    metadata = listed["videos_rate"]["metadata"]
    description = listed["videos_rate"]["description"]
    example_names = {example["name"] for example in metadata["examples"]}
    metadata_text = " ".join([description, *metadata["usageNotes"], *metadata["caveats"]])
    ...
    result = dispatcher.call_tool("videos_rate", {"id": "abc123", "rating": "like"})

    assert result["endpoint"] == "videos.rate"
    assert result["quotaCost"] == 50
    assert result["auth"] == {"mode": "oauth_required", "path": "restricted"}
    assert result["rating"] == {"videoId": "abc123", "requestedRating": "like"}
...
def test_default_registry_includes_executable_playlists_insert_tool_with_insert_metadata():
    """Register ``playlists_insert`` by default with insert metadata."""
    dispatcher = InMemoryToolDispatcher()
    listed = {tool["name"]: tool for tool in dispatcher.list_tools()}

    assert "playlists_insert" in listed
    metadata = listed["playlists_insert"]["metadata"]
    description = listed["playlists_insert"]["description"]
    metadata_text = " ".join([description, *metadata["usageNotes"], *metadata["caveats"]])

...
class Layer1FoundationIntegrationTests(unittest.TestCase):
    ...
    def test_i18n_languages_list_wrapper_accepts_default_display_language_requests(self):
        wrapper = build_i18n_languages_list_wrapper()
        executor = IntegrationExecutor(
            transport=lambda execution: {
                "items": [{"id": "en", "kind": "youtube#i18nLanguage"}],
        ...
        self.assertEqual(result["items"][0]["id"], "en")
        self.assertEqual(result["arguments"], {"part": "snippet"})
    ...
    def test_i18n_regions_list_wrapper_accepts_missing_display_language_requests(self):
        wrapper = build_i18n_regions_list_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: {"items": []},
            retry_policy=RetryPolicy(max_attempts=1),
    ...
    def test_videos_update_wrapper_executes_authorized_requests_through_shared_executor(self):
        wrapper = build_videos_update_wrapper()
        executor = IntegrationExecutor(
            transport=lambda execution: {
                "id": execution.arguments["body"]["id"],
                "title": execution.arguments["body"]["snippet"]["title"],
    ...
    def test_videos_update_wrapper_preserves_part_and_title_in_successful_results(self):
        wrapper = build_videos_update_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: {
                "id": "video-123",
    ...
    def test_videos_update_wrapper_preserves_invalid_request_failures_from_shared_executor(self):
        wrapper = build_videos_update_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: (_ for _ in ()).throw(
                normalize_upstream_error(
                    RuntimeError("Video update payload invalid"),
                    category="invalid_request",
                    status_code=400,
                    details={"reason": "required"},
                )
        ...
        with self.assertRaisesRegex(NormalizedUpstreamError, "Video update payload invalid") as context:
            wrapper.call(
                executor,
                arguments={
                    "part": "snippet",
    ...
    def test_videos_update_wrapper_preserves_auth_failures_from_shared_executor(self):
        wrapper = build_videos_update_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: (_ for _ in ()).throw(
                normalize_upstream_error(
    ...
    def test_videos_update_wrapper_preserves_upstream_update_failures_from_shared_executor(self):
        wrapper = build_videos_update_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: (_ for _ in ()).throw(
                normalize_upstream_error(
                    RuntimeError("Video update rejected by policy"),
                    status_code=409,
                    details={"reason": "conflict"},
                )
            ),
        ...
        with self.assertRaisesRegex(
            NormalizedUpstreamError,
            "Video update rejected by policy",
        ) as context:
            wrapper.call(
                executor,
                arguments={
    ...
    def test_videos_report_abuse_wrapper_executes_authorized_requests_through_shared_executor(self):
    ...
    def test_videos_report_abuse_wrapper_rejects_local_validation_failures_before_executor(self):
        wrapper = build_videos_report_abuse_wrapper()
        executor = IntegrationExecutor(
        ...
        with self.assertRaisesRegex(ValueError, "body.reasonId is required"):
            wrapper.call(
                executor,
                arguments={"body": {"videoId": "video-123"}},
                auth_context=AuthContext(
                    mode=AuthMode.OAUTH_REQUIRED,
                    credentials=CredentialBundle(oauth_token="oauth-123"),
                ),
    ...
    def test_videos_report_abuse_wrapper_preserves_access_failures_from_shared_executor(self):
        wrapper = build_videos_report_abuse_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: (_ for _ in ()).throw(
                normalize_upstream_error(
                    RuntimeError("Report abuse access denied"),
                    category="auth",
                    status_code=403,
                    details={"reason": "forbidden"},
                )
        ...
        with self.assertRaisesRegex(NormalizedUpstreamError, "Report abuse access denied") as context:
            wrapper.call(
                executor,
                arguments={"body": {"videoId": "video-123", "reasonId": "spam"}},
                auth_context=AuthContext(
    ...
    def test_videos_report_abuse_wrapper_preserves_upstream_refusal_failures(self):
        wrapper = build_videos_report_abuse_wrapper()
        executor = IntegrationExecutor(
            transport=lambda _execution: (_ for _ in ()).throw(
                normalize_upstream_error(
                    RuntimeError("invalid abuse reason"),
                    category="invalid_request",
                    status_code=400,
                    details={"reason": "invalidAbuseReason"},
                )
            ),
            retry_policy=RetryPolicy(max_attempts=1),
        )

        with self.assertRaisesRegex(NormalizedUpstreamError, "invalid abuse reason") as context:
            wrapper.call(
                executor,

"""Contract tests for the Layer 2 ``i18nLanguages_list`` tool."""
...
def test_i18n_languages_list_public_symbols_are_exported():
    """Expose ``i18nLanguages_list`` symbols from the shared package."""
    ...
    assert I18N_LANGUAGES_LIST_TOOL_NAME == "i18nLanguages_list"
    assert callable(localization.build_i18n_languages_list_tool_descriptor)


def test_i18n_languages_list_schema_preserves_part_and_optional_display_language():
    """Expose the upstream-like request fields for ``i18nLanguages_list``."""
...
def test_i18n_languages_list_public_contract_identifies_active_endpoint():
...
def test_i18n_languages_list_descriptor_uses_public_contract_shape():
    """Build an executable descriptor aligned with the public contract."""
    descriptor = build_i18n_languages_list_tool_descriptor()

    assert descriptor["name"] == "i18nLanguages_list"
    assert descriptor["inputSchema"] == I18N_LANGUAGES_LIST_INPUT_SCHEMA
    assert descriptor["metadata"]["upstream"]["operationKey"] == "i18nLanguages.list"
    assert descriptor["metadata"]["quotaCost"] == 1


def test_i18n_languages_list_metadata_exposes_quota_auth_examples_and_boundaries():
    """Expose safe caller-facing metadata before ``i18nLanguages_list`` invocation."""
    from mcp_server.tools.youtube_common.localization import I18N_LANGUAGES_LIST_CALLER_EXAMPLES
    ...
    example_names = {example["name"] for example in I18N_LANGUAGES_LIST_CALLER_EXAMPLES}
...
def test_i18n_languages_list_descriptor_metadata_examples_and_errors_are_secret_free():
    """Keep public ``i18nLanguages_list`` metadata free of unsafe diagnostics."""
    descriptor = build_i18n_languages_list_tool_descriptor()
    public_payload = {

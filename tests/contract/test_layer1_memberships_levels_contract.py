...
from mcp_server.integrations.wrappers import build_memberships_levels_list_wrapper


class Layer1MembershipsLevelsContractTests(unittest.TestCase):
    def _feature_contract_root(self) -> str:
        return os.path.abspath("specs/127-memberships-levels-list/contracts")

    def test_contract_artifacts_define_wrapper_and_owner_visibility_guidance(self):
        root = self._feature_contract_root()
        with open(
            os.path.join(root, "layer1-memberships-levels-list-wrapper-contract.md"),
            "r",
            encoding="utf-8",
        ) as handle:
            wrapper_contract = handle.read()
        with open(
            os.path.join(root, "layer1-memberships-levels-list-owner-visibility-contract.md"),
            "r",
            encoding="utf-8",
        ) as handle:
        ...
        self.assertIn("unsupported", owner_contract.lower())

    def test_memberships_levels_list_wrapper_review_surface_exposes_identity_quota_and_owner_notes(self):
    ...
    def test_contract_documents_request_boundaries_and_invalid_request_rules(self):
        ...
        self.assertIn("`part` is required", wrapper_contract)
        self.assertIn("unsupported fields", wrapper_contract)
    ...
    def test_contract_documents_owner_visibility_and_empty_result_rules(self):
        ...
        self.assertIn("owner-only", owner_contract.lower())
        self.assertIn("empty membership-level results", owner_contract.lower())

...
def test_subscriptions_delete_contract_uses_existing_resource_family():
    ...
    assert subscriptions_family.definition_location.endswith("src/mcp_server/tools/youtube_common/subscriptions.py")
    assert subscriptions_family.layer1_dependency == "mcp_server.integrations.resources.subscriptions"
    assert subscriptions.SUBSCRIPTIONS_DELETE_TOOL_NAME == "subscriptions_delete"
    assert subscriptions.SUBSCRIPTIONS_DELETE_QUOTA_COST == 50
    assert youtube_common.SUBSCRIPTIONS_DELETE_TOOL_NAME == "subscriptions_delete"
    assert youtube_common.SUBSCRIPTIONS_DELETE_QUOTA_COST == 50
    assert callable(youtube_common.build_subscriptions_delete_tool_descriptor)
...
def test_videos_rate_contract_uses_existing_resource_family():
    ...
    assert youtube_common.VIDEOS_RATE_INPUT_SCHEMA["required"] == ["id", "rating"]
    assert youtube_common.VideosRateToolError is videos.VideosRateToolError
    assert callable(youtube_common.build_videos_rate_contract)
    assert callable(youtube_common.build_videos_rate_handler)
    assert callable(youtube_common.build_videos_rate_tool_descriptor)
    assert callable(youtube_common.map_videos_rate_result)
    assert callable(youtube_common.validate_videos_rate_arguments)
    assert metadata["name"] == "videos_rate"
    assert metadata["resourceFamily"] == "videos"
...
def test_videos_delete_contract_uses_existing_resource_family():
    ...
    assert callable(youtube_common.build_videos_delete_contract)
    assert callable(youtube_common.build_videos_delete_handler)
    assert callable(youtube_common.build_videos_delete_tool_descriptor)
    assert callable(youtube_common.map_videos_delete_result)
    assert callable(youtube_common.validate_videos_delete_arguments)
    assert metadata["name"] == "videos_delete"
    assert metadata["resourceFamily"] == "videos"

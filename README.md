...
- `YOUTUBE_OAUTH_TOKEN` is an optional opaque OAuth access token for
  OAuth-required YouTube operations. This service consumes an operator-managed
  token; obtaining or refreshing it is outside the server. Keep it in Secret
  Manager, never in tool input, logs, errors, or committed files.
...
- `SECRET_REFERENCES`: comma-separated Secret Manager secret names injected into
  ...
  `MCP_AUTH_TOKEN`; add `YOUTUBE_OAUTH_TOKEN` when OAuth-required operations
  are enabled.

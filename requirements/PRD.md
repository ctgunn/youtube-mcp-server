...
- Product layers for YouTube capability:
  ...
  - Layer 4: the internal MCP integration-verification layer that discovers the registered catalog and exercises each public tool through the actual MCP request route with deterministic, non-production fixtures.
...
- The production execution path MUST conform to the current official YouTube Data API request protocol for every supported operation. Endpoint metadata and tests MUST distinguish a standard resource request from a media-upload request, including the required upload origin/path, query parameters, headers, request body encoding, and response shape.
- The production execution path MUST support the HTTP methods, query parameters, JSON request bodies, raw-media uploads, multipart uploads, and resumable uploads required by the supported endpoint inventory. A public tool MUST NOT advertise a resumable upload mode unless the runtime implements session initiation, upload-URL handling, chunk transfer or recovery, and final-response handling for that mode.
- Media inputs MUST have a bounded, documented, and transport-safe representation. Large media uploads MUST stream or use the resumable protocol; the service MUST NOT require an entire large video to be materialized as an in-memory MCP argument.
- The production execution path MUST attach the applicable API-key or OAuth credential, preserve observability hooks, prevent credential values from appearing in logs or error messages, and normalize upstream HTTP and payload errors into the shared error model. OAuth-backed operations require a deployable credential lifecycle, including configured scopes and renewal or rotation before an access token expires.
- Retry behavior MUST use bounded exponential backoff with full jitter and honor applicable upstream retry guidance. It MUST NOT automatically replay a non-idempotent mutation after an ambiguous failure unless the operation has a documented safe recovery or idempotency strategy.
...
- A runtime that advertises an OAuth-required tool as available MUST have the required OAuth capability configured. Hosted configuration and readiness reporting MUST distinguish API-key-only read capability from OAuth-backed owner, mutation, and media-upload capability.
...
- Each resource-family retrofit MUST prove the request method, path, parameters, credential mode, request body or upload form where applicable, successful-response mapping, and normalized failure behavior for every endpoint method in that family. Controlled test doubles are appropriate for deterministic coverage, but they are insufficient on their own: a credential-gated live smoke suite MUST verify representative read, OAuth, and media paths against the real YouTube Data API without exposing secrets or mutating production data.
...
### 6.2.2 Layer 4 MCP Integration Verification Requirements
- Layer 4 MUST maintain a catalog-driven integration suite that obtains the tool inventory through the public MCP `tools/list` flow and invokes every registered default tool through the public MCP `tools/call` flow.
- The suite MUST give every discovered tool an individually reported test case, a valid deterministic fixture, and an asserted MCP outcome; a loop that hides which tool failed is insufficient.
- For tools whose deterministic no-runtime fixture intentionally has no remote resource data, the suite MUST assert the documented, MCP-safe error category rather than manufacture a success response.
- For endpoint-backed tools that return a normalized `endpoint` field, the suite MUST verify that the returned endpoint agrees with the tool's registered upstream operation metadata.
...
- Every default public MCP tool has deterministic Layer 4 discovery-and-invocation coverage, with unavailable local-data paths mapped to protocol-safe errors rather than process failures.
...
20. Complete the shared authenticated live YouTube Data API execution path and replace representative default execution in all existing resource-family wrappers, grouped for delivery by resource family. This includes Google-conformant media and resumable-upload protocols, OAuth capability lifecycle, mutation-safe retry behavior, and streaming-safe media handling.
21. Verify that every configured Layer 1, Layer 2, and higher-level YouTube request reaches the live execution path or fails safely when configuration or credentials are unavailable, using both deterministic request-conformance tests and credential-gated real-API smoke verification.

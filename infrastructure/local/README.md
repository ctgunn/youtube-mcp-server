...
If hosted-like local verification is selected without the Redis dependency running, start `docker compose -f infrastructure/local/compose.yaml up -d` or `./scripts/local_compose.sh up -d` first and then rerun `LOCAL_SESSION_MODE=hosted bash scripts/dev_local.sh`.
